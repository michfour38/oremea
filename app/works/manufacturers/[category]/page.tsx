...
import { worksEvidencePublicLabel } from "@/lib/works/evidence/freshness";
  ...
  const providerMarkets = await prisma.works_provider_markets.findMany({
    ...
    select: {
      ...
      offerings: {
        ...
        select: { name: true, evidence_status: true, updated_at: true },
        orderBy: { name: "asc" },
        take: 4,
      },
    },
      ...
      <div className="mx-auto w-full max-w-5xl px-5 pt-6 md:px-8 md:pt-9">
        ...
        <section className="border-t border-black/10 py-8">
                  ...
                  <div className="mt-4 space-y-2 border-t border-black/8 pt-4">
                    {item.offerings.map((offering) => (
                      <div key={offering.name} className="flex items-start justify-between gap-3 text-xs">
                        ...
                        <span className="shrink-0 text-right text-black/40">
          ...
          <p className="mt-5 text-xs leading-5 text-black/40">
            A public listing is a possible fit, not a guarantee of current capacity, specification, price or timing. Reviewed evidence that has not been refreshed recently is marked for reconfirmation. Confirm appointment-critical facts directly before choosing a provider.
          </p>
        </section>
      </div>
