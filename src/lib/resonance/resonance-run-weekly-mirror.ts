...
function dailyMirrorFromSnapshot(snapshot: unknown) {
  ...
  const dailyMirror = (value as Record<string, unknown>).dailyMirror;
  return typeof dailyMirror === "string" ? dailyMirror.trim() : "";
}
...
async function getGuidanceContext(runs: JourneyRun[]) {
  ...
  return rows
    .map((row) => ({
      ...
      dailyMirror: dailyMirrorFromSnapshot(row.input_snapshot),
    }))
...
function buildCurrentDailyMirrorContext(
  guidance: Awaited<ReturnType<typeof getGuidanceContext>>,
  currentRunId: string,
) {
  const mirrors = guidance.filter(
    (row) => row.runId === currentRunId && row.dailyMirror,
  );

  if (mirrors.length === 0) return "None yet.";

  return mirrors
    .map((row) => `[Day ${row.dayNumber} Daily Mirror]\n${row.dailyMirror}`)
    .join("\n\n---\n\n");
}
...
function buildPrompt(params: {
  ...
  const currentDailyMirrorContext = buildCurrentDailyMirrorContext(
    guidance,
    currentRunId,
  );
  ...
  return `
You are the Resonance Closing Mirror.

Resonance means: Help me stay with myself.

This Closing Mirror closes Week ${currentWeekNumber} · Run ${currentRunNumber} while retaining continuity with the participant's earlier completed Resonance visits.
...
THE CLOSING MIRROR'S WAY OF SEEING
This is a fresh synthesis across all seven days of the visit, not a compilation of the seven Daily Mirrors.
The participant's raw reflections across Days 1–7 and their participant-written 2Q answers are primary evidence.
Daily Mirrors are anti-repetition context only. They tell you what has already been reflected; they are never evidence about the participant.
Do not paraphrase, recap, repackage, or repeat observations already made in the first six Daily Mirrors. If something already reflected earlier matters at the close, move beyond the earlier observation only when the seven-day evidence reveals a genuinely new relationship, recurrence, contrast, shift, consequence, or distinction.
...
Current-run participant writing across all seven days has foreground authority.
...
Generated 2Q questions, Daily Mirrors, and prior Closing Mirrors are continuity context only. They may tell you what has already been reflected or asked; they never prove anything about the participant.
Participant-written 2Q answers are participant evidence.
...
The Closing Mirror does not need a unifying theory.

YOUR TASK
Read the seven-day body of participant evidence as one visit and reflect what becomes visible only because the days can now be heard together.
Do not walk through Day 1, then Day 2, then Day 3. Do not produce a seven-day recap.
Look for relationships between days, recurring language or behaviour, contrasts, shifts, consequences, and distinctions that were not available from one day alone.
Use earlier completed visits only where earned and only after the current seven-day visit has been heard accurately.

Notice:
- what is fresh, alive, specific, or newly clarified across the current seven-day visit
- what the participant explicitly says matters
- a recurrence that the evidence genuinely supports across days
...
- what becomes visible only when two or more days are placed beside one another
...
Write roughly 6 to 9 short paragraphs, using only as much length as the seven-day evidence earns.
Begin with the most alive specific thing from the run closing now.
Build a fresh cross-day synthesis rather than revisiting the sequence of Daily Mirrors.
Give particular attention to what is newly available to see because all seven days now exist together.
...
CURRENT SEVEN-DAY VISIT — FOREGROUND PRIMARY EVIDENCE
...
CURRENT VISIT DAILY MIRRORS — ANTI-REPETITION CONTEXT ONLY
Use these only to know what has already been said. Do not repeat or paraphrase them in the Closing Mirror.
${currentDailyMirrorContext}

FULL PARTICIPANT REFLECTION TIMELINE — SUPPORTING PRIMARY EVIDENCE
Journey order: ${journeyOrder}
${reflectionTimeline}

PRIOR CLOSING MIRROR CONTEXT
Use only to curb repetition and understand what has already been reflected in earlier completed visits. Do not use these outputs as evidence about the participant.
${mirrorContext}
`.trim();
}
...
export async function runResonanceRunWeeklyMirror(
  ...
  const currentRunDailyMirrorCount = guidance.filter(
    (row) => row.runId === activeRun.id && row.dailyMirror,
  ).length;
  ...
  const inputSnapshot = JSON.stringify({
    ...
    continuityContext:
      "current_daily_mirrors_for_anti_repetition_generated_2q_and_prior_closing_mirrors",
    currentRunId: activeRun.id,
    currentWeekNumber: activeRun.weekNumber,
    currentRunNumber: activeRun.runNumber,
