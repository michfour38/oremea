...
export function ProviderOutreachPanel({
  ...
  function normalizePreview(preview: EmailPreview) {
  ...
  async function fetchPreviews(providerIds: string[]) {
    ...
    return (data.previews ?? []) as EmailPreview[];
  }
  ...
  async function review() {
    try {
      ...
      const freshPreviews = await fetchPreviews(selectedContactable);
  ...
  async function restoreOriginal(providerId: string) {
    ...
    try {
      ...
      setStatus("RESETTING");
      setResettingProviderId(providerId);
      const freshPreviews = await fetchPreviews([providerId]);
      const fresh = freshPreviews.find(
        (preview) => preview.providerId === providerId
      );
      if (!fresh) {
        throw new Error("WORKS could not regenerate this provider email");
      }

      const restored = normalizePreview(fresh);
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "WORKS could not restore this draft"
      );
    } finally {
