...
export function WorksProviderOnboarding() {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(JOURNEY_KEY, JSON.stringify({ tab, mode, query, businessEmail, note, created } satisfies SavedJourney));
  ...
  async function createProvider() {
    try {
      ...
      setMessage(data.message ?? "Your WORKS provider profile is created.");
      window.localStorage.removeItem(FORM_KEY);
      setTab("progress");
    } catch (err) {
  ...
  useEffect(() => {
    ...
    // The signed-in redirect reloads this page, so one load per selected claim path is sufficient.
  ...
  const tabs: { id: Tab; label: string; hint: string; disabled?: boolean }[] = [
    ...
    { id: "plans", label: "Plans", hint: "Reference" },
      ...
      <div className="mx-auto w-full max-w-6xl px-5 py-8 md:px-8 md:py-12">
            ...
            <>
              <SignedOut>
                <SignInButton mode="modal" forceRedirectUrl={returnUrl}>
                  <button className="rounded-full border border-black/15 bg-white px-4 py-2 text-sm">Provider sign in</button>
                </SignInButton>
              </SignedOut>
              <SignedIn>
                <a href="/works/provider" className="rounded-full border border-black/15 bg-white px-4 py-2 text-sm">Provider workspace →</a>
              </SignedIn>
            </>
          }
        />

        <nav className="mt-7 grid gap-2 sm:grid-cols-4" aria-label="Provider joining progress" role="tablist">
          {tabs.map((item) => {
        ...
        {ready && tab === "start" ? (
          <section className="py-12 md:py-16" role="tabpanel">
            <p className="text-xs font-medium uppercase tracking-[0.2em] text-[#16834f]">Matching capacity with opportunity</p>
            <h1 className="mt-4 max-w-4xl font-serif text-5xl leading-[1.05] md:text-6xl">Bring the work you want more of into WORKS</h1>
            <p className="mt-6 max-w-2xl text-base leading-8 text-black/60">Begin here, move through the business step, compare plans whenever needed, and return to your progress without leaving this page.</p>
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-2">
              <button type="button" onClick={() => chooseMode("add")} className="rounded-3xl bg-[#1f1c17] p-7 text-left text-white">
                ...
                <span className="mt-3 block max-w-md text-sm leading-6 text-white/65">Create a provider record connected to your account. Your information remains available as you move between tabs.</span>
              ...
              <button type="button" onClick={() => chooseMode("claim")} className="rounded-3xl border border-black/12 bg-white p-7 text-left">
                ...
                <span className="mt-3 block max-w-md text-sm leading-6 text-black/50">Search for an existing WORKS profile and submit the connection from the same joining space.</span>
                <span className="mt-6 block text-sm">Continue →</span>
              </button>
            </div>

            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 border-t border-black/10 pt-6 text-sm text-black/50">
              <button type="button" onClick={() => setTab("plans")} className="underline underline-offset-4">Compare provider plans</button>
          ...
          <section className="py-10 md:py-14" role="tabpanel">
            <div className="flex flex-wrap items-end justify-between gap-5 border-b border-black/10 pb-6">
              <div>
                ...
                <h1 className="mt-3 font-serif text-4xl md:text-5xl">Tell WORKS which business you represent</h1>
              </div>
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => chooseMode("claim")} className={`rounded-full border px-4 py-2 text-sm ${mode === "claim" ? "border-[#1f1c17] bg-[#1f1c17] text-white" : "border-black/15 bg-white"}`}>Find a business</button>
              </div>
            </div>

            {!mode ? <div className="mt-8 rounded-3xl border border-black/10 bg-white/70 p-7 text-sm text-black/55">Choose whether you are adding a new provider or connecting to a business already listed on WORKS.</div> : null}
            ...
            <SignedOut>
              {mode ? (
                <div className="mt-8 rounded-3xl border border-black/10 bg-white p-7 md:p-8">
                  ...
                  <p className="mt-4 max-w-xl text-sm leading-7 text-black/55">WORKS returns to this business tab after sign-in. Anything already entered remains here.</p>
                  <SignInButton mode="modal" forceRedirectUrl={returnUrl}>
            ...
            <SignedIn>
              {mode === "add" ? (
                <div className="mt-8 grid gap-5 rounded-3xl border border-black/10 bg-white p-6 md:p-8">
                  <div>
                    <h2 className="font-serif text-3xl">Add your manufacturing or production business</h2>
                    <p className="mt-3 max-w-2xl text-sm leading-7 text-black/55">Contact details, description and location remain private by default until you choose to publish them from your provider workspace.</p>
                  </div>
                  ...
                  <label className="text-xs text-black/45">Business description<textarea value={form.description} onChange={(event) => setForm(value => ({ ...value, description: event.target.value }))} rows={5} placeholder="What do you manufacture, supply or help bring into production?" className="mt-1 w-full resize-none rounded-xl border border-black/10 px-4 py-3 text-sm leading-6 outline-none focus:border-[#16834f]" /></label>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    ...
                    <label className="flex items-start gap-3 rounded-2xl border border-black/10 p-4"><input type="checkbox" checked={form.acceptsRemoteClients} onChange={(event) => setForm(value => ({ ...value, acceptsRemoteClients: event.target.checked }))} className="mt-1"/><span><span className="block text-sm font-medium">Accept remote clients</span><span className="mt-1 block text-xs leading-5 text-black/45">Used by WORKS when building viable production routes.</span></span></label>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <button type="button" onClick={createProvider} disabled={saving || form.name.trim().length < 2} className="rounded-full bg-[#1f1c17] px-6 py-3 text-sm text-white disabled:opacity-40">{saving ? "Creating…" : "Add my business →"}</button>
                    <button type="button" onClick={() => chooseMode("claim")} className="text-sm underline underline-offset-4 text-black/55">My business is already on WORKS</button>
                  </div>
                </div>
              ) : null}
              ...
              {mode === "claim" ? (
                <div className="mt-8">
                  <div className="rounded-3xl border border-black/10 bg-white p-6 md:p-8">
                    ...
                    <p className="mt-3 max-w-2xl text-sm leading-7 text-black/55">Search by business name. Editing access begins after WORKS verifies the relationship.</p>
                    ...
                    {results.length ? <div className="mt-6 space-y-3">{results.map((provider) => <button key={provider.id} type="button" disabled={provider.alreadyClaimed} onClick={() => { setSelected(provider); setMessage(""); setError(""); }} className="flex w-full items-center justify-between gap-4 rounded-2xl border border-black/10 bg-white p-5 text-left disabled:opacity-45"><div><p className="font-medium">{provider.name}</p>{provider.website ? <p className="mt-1 text-xs text-black/40">{provider.website}</p> : null}</div><span className="text-sm text-black/45">{provider.alreadyClaimed ? "Already managed" : "Connect →"}</span></button>)}</div> : query.trim().length >= 2 && !loading ? <div className="mt-6 rounded-2xl border border-black/10 bg-[#fbfaf7] p-5"><p className="text-sm">No matching WORKS profile found</p><button type="button" onClick={() => chooseMode("add")} className="mt-3 text-sm underline underline-offset-4">Add a new provider profile →</button></div> : null}
                  </div>

                  {selected ? <section className="mt-6 rounded-3xl border border-black/10 bg-white p-6 md:p-7"><p className="text-xs font-medium uppercase tracking-[0.18em] text-[#16834f]">Connect to {selected.name}</p><h2 className="mt-3 font-serif text-3xl">Confirm your connection to this business</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-black/50">Use an email address on the business&apos;s own domain. Personal email services such as Gmail cannot establish ownership. Where WORKS has the company website, the email domain must match it.</p><div className="mt-6 grid gap-4"><label className="text-xs text-black/45">Business email<input type="email" value={businessEmail} onChange={(event) => setBusinessEmail(event.target.value)} placeholder="you@company.co.za" className="mt-1 w-full rounded-xl border border-black/10 bg-white px-4 py-3 text-base outline-none focus:border-[#16834f]" /></label><label className="text-xs text-black/45">Your role or anything that helps WORKS verify the connection<textarea value={note} onChange={(event) => setNote(event.target.value)} rows={3} placeholder="Owner, operations manager, company director…" className="mt-1 w-full resize-none rounded-xl border border-black/10 bg-white px-4 py-3 text-sm leading-6 outline-none focus:border-[#16834f]" /></label></div><p className="mt-4 text-xs leading-5 text-black/40">The email and verification note stay inside WORKS. A pending request has zero editing authority.</p><div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={() => void submitClaim()} disabled={sending || !businessEmail.trim()} className="rounded-full bg-[#1f1c17] px-6 py-3 text-sm text-white disabled:opacity-40">{sending ? "Submitting…" : "Submit connection request →"}</button><button type="button" onClick={() => setSelected(null)} className="rounded-full border border-black/15 bg-white px-5 py-3 text-sm">Cancel</button></div></section> : null}

                  {claims.length ? <section className="mt-8 border-t border-black/10 pt-8"><p className="text-xs font-medium uppercase tracking-[0.18em] text-black/40">Your connection requests</p><div className="mt-4 space-y-3">{claims.map((claim) => <div key={claim.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white/70 p-4"><div><p className="text-sm font-medium">{claim.provider.name}</p><p className="mt-1 text-xs text-black/40">Submitted with {claim.business_email}</p></div><span className="rounded-full border border-black/10 px-3 py-1.5 text-xs text-black/55">{claim.status.charAt(0) + claim.status.slice(1).toLowerCase()}</span></div>)}</div></section> : null}
        ...
        {ready && tab === "plans" ? (
          <section className="py-10 md:py-14" role="tabpanel">
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 text-black/55">Plans remain here for comparison while you join. Profile creation and connection can continue without leaving this page.</p>
          ...
          <section className="py-10 md:py-14" role="tabpanel">
            ...
            {created ? (
              <div className="mt-4 rounded-3xl border border-black/10 bg-white p-7 md:p-9">
                ...
                <p className="mt-5 max-w-2xl text-sm leading-7 text-black/55">Open the provider workspace to choose what customers can see, add capacity, and tell WORKS what work you want more of.</p>
                <div className="mt-8 flex flex-wrap gap-3"><a href="/works/provider" className="rounded-full bg-[#1f1c17] px-6 py-3 text-sm text-white">Open provider workspace →</a><a href={`/works/providers/${created.slug}`} className="rounded-full border border-black/15 bg-white px-6 py-3 text-sm">View public profile</a><button type="button" onClick={startAnother} className="rounded-full border border-black/15 bg-white px-6 py-3 text-sm">Add another business</button></div>
              </div>
            ) : null}

            {!created && message ? <div className="mt-4 rounded-3xl bg-[#eef7f1] p-7 text-sm leading-7">{message}</div> : null}

            {claims.length ? <section className="mt-8"><h2 className="font-serif text-3xl">Connection requests</h2><div className="mt-5 space-y-3">{claims.map((claim) => <div key={claim.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-black/10 bg-white p-5"><div><p className="text-sm font-medium">{claim.provider.name}</p><p className="mt-1 text-xs text-black/40">Submitted with {claim.business_email}</p></div><span className="rounded-full border border-black/10 px-3 py-1.5 text-xs text-black/55">{claim.status.charAt(0) + claim.status.slice(1).toLowerCase()}</span></div>)}</div></section> : null}

            {!created && !message && !claims.length ? <div className="mt-4 rounded-3xl border border-black/10 bg-white/70 p-7 text-sm text-black/50">Your completed profile or connection request will remain available here.</div> : null}

            <div className="mt-8 flex flex-wrap gap-3"><button type="button" onClick={() => setTab("business")} className="rounded-full border border-black/15 bg-white px-5 py-2.5 text-sm">Return to business details</button><button type="button" onClick={() => setTab("plans")} className="rounded-full border border-black/15 bg-white px-5 py-2.5 text-sm">Review plans</button></div>
          </section>
        ) : null}
      </div>
    </main>
