...
export function WorksProviderInbox() {
  ...
  async function save() {
    ...
    try {
      ...
      setMessage("Response saved. WORKS can now use this information in the production route.");
    } catch (err) {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-6xl px-5 py-8 md:px-8 md:py-12">
    <header className="flex items-center justify-between border-b border-black/10 pb-5">
      <div><a href="/works/za" className="text-xs font-semibold uppercase tracking-[0.32em] text-[#16834f]">WORKS</a><p className="mt-1 text-xs text-black/40">Provider inbox</p></div>
      <div className="flex items-center gap-4"><SignedIn><a href="/works/provider" className="text-sm underline underline-offset-4">Provider profile</a><UserButton afterSignOutUrl="/works/za" /></SignedIn></div>
    </header>
    ...
    <SignedIn>{loading ? <p className="py-12 text-sm text-black/40">Loading opportunities…</p> : items.length === 0 ? <section className="py-14"><h1 className="font-serif text-4xl">Provider inbox</h1><p className="mt-4 max-w-xl text-sm leading-7 text-black/50">No WORKS production briefs have been sent to your provider profile yet.</p></section> : <section className="grid gap-6 py-10 lg:grid-cols-[.8fr_1.2fr]">
      <div className="space-y-3">
        <div className="mb-5"><p className="text-xs font-medium uppercase tracking-[0.18em] text-[#16834f]">Incoming production briefs</p><h1 className="mt-2 font-serif text-3xl">Provider inbox</h1></div>
        {items.map((item) => <button key={item.id} type="button" onClick={() => choose(item)} className={`block w-full rounded-2xl border p-5 text-left ${selected?.id === item.id ? "border-[#1f1c17] bg-white" : "border-black/10 bg-white/60"}`}><p className="text-xs uppercase tracking-[0.12em] text-black/35">{item.provider.name} · {pretty(item.status)}</p><p className="mt-2 font-serif text-xl">{item.brief.productDescription}</p><p className="mt-3 text-xs text-black/45">{item.relevantSteps.length ? item.relevantSteps.join(" · ") : "Production opportunity"}</p></button>)}
      </div>

      {selected && draft ? <div className="rounded-3xl border border-black/10 bg-white/75 p-6 md:p-8">
        <p className="text-xs uppercase tracking-[0.18em] text-[#16834f]">{selected.provider.name}</p><h2 className="mt-2 font-serif text-3xl">{selected.brief.productDescription}</h2>
        <div className="mt-6 grid gap-3 text-sm text-black/55 sm:grid-cols-2">
          ...
          {selected.relevantSteps.length ? <p><span className="block text-xs text-black/35">WORKS needs you for</span>{selected.relevantSteps.join(", ")}</p> : null}
        </div>

        <div className="mt-8 border-t border-black/8 pt-6"><p className="text-sm font-medium">Does this fit your capability?</p><div className="mt-3 flex flex-wrap gap-2">{[["YES","Yes"],["POSSIBLE","Possibly · need more information"],["OUTSIDE_CAPABILITY","Outside our capability"]].map(([value,label]) => <button key={value} type="button" onClick={() => setDraft((current) => current ? { ...current, decision: value as Draft["decision"] } : current)} className={`rounded-full border px-4 py-2.5 text-sm ${draft.decision === value ? "border-[#1f1c17] bg-[#1f1c17] text-white" : "border-black/15 bg-white"}`}>{label}</button>)}</div></div>
        ...
        <p className="mt-3 text-xs leading-5 text-black/35">This response belongs to this production brief. It does not silently rewrite your provider-wide profile or public information.</p>
      </div> : null}
    </section>}</SignedIn>
  </main>;
}
