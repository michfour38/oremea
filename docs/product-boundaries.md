...
5. Participant-facing intelligence must remain inside `src/lib/oremea/participant-sovereignty.ts`. Existing consumers of `OREMEA_EVIDENCE_BOUNDARY` inherit the universal sovereignty contract; shared EL conversations also apply the active product's sovereignty extension. Product-specific prompts may narrow these boundaries but may not grant authority the central contract withholds.
