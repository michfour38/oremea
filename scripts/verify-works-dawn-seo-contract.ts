...
const evidenceFreshness = read("lib/works/evidence/freshness.ts");
...
assert.match(profile, /worksEvidencePublicLabel/);
assert.match(profile, /isWorksEvidenceFresh\(offering\.updated_at, now\)/);
assert.match(profile, /evidence_status !== "SELF_REPORTED"/);
assert.match(profile, /structuredData\.makesOffer/);
assert.match(evidenceFreshness, /Provider supplied · review pending/);
assert.match(evidenceFreshness, /Previously reviewed · needs reconfirmation/);
