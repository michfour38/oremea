...
requireText(providerOnboarding, '"Request access →"', "A previously managed listing must still offer a verified business-domain access path.");
...
requireText(providerClaimsApi, "accountEmail === businessEmail", "Automatic claim approval must require the signed-in verified business email.");
requireText(providerClaimsApi, "provider.memberships.length > 0 && !isQaProvider && !verifiedDomainOwner", "An existing manager must remain protected from unrelated public claims.");
...
requireText(providerClaimsApi, "accessGranted: true", "A verified business-domain claim must return direct provider access.");
